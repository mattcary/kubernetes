...
func TestStatefulPodControlClaimsMatchDeletionPolcy(t *testing.T) {
	...
	testFn := func(t *testing.T) {
		...
		setDeletePolicy := apps.DeleteOnStatefulSetDeletionOnlyPersistentVolumeClaimDeletePolicyType
		set.Spec.PersistentVolumeClaimDeletePolicy = &setDeletePolicy
...
func TestStatefulPodControlDeletionPolicyUpdateMissingClaims(t *testing.T) {
	...
	control := NewStatefulPodControl(fakeClient, podLister, claimLister, recorder)
	if err := control.UpdateStatefulPod(set, pod); err == nil {
		t.Error("Expected error on pod update when PVCs are missing")
	}
	claims := getPersistentVolumeClaims(set, pod)
	if len(claims) != 1 {
		t.Errorf("Unexpected or missing PVCs")
	...
	if !strings.Contains(events[0], "FailedUpdate") {
		t.Errorf("expected first event to be a failed update: %s", events[0])
	}
	if !strings.Contains(events[1], "SuccessfulUpdate") {
